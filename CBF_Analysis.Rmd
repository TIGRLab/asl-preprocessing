...
```{r}
boxplot(WMmeanCBF~assigned_sex_at_birth,data=data_final,xlab= "Biological Sex", ylab= "Mean GM", par(cex.lab=1.5), par(cex.axis=1.5))
```
```{r}
t.test(WMmeanCBF ~ assigned_sex_at_birth, mu=0, alt="two.sided", conf=0.95, data=data_final)
```
...
```{r}
ggplot(data = data_final, aes(x = age_scan, y = WMmeanCBF)) + geom_point() + geom_smooth(method = "lm") + facet_grid(~assigned_sex_at_birth) + xlab("Age") + ylab("Mean GM") + theme(text = element_text(size = 20)) 
```

```{r}
cor(data_final$age_scan , data_final$WMmeanCBF)
```



```{r}
ggplot(data = data, aes(x = assigned_sex_at_birth, y = WMmeanCBF))+ xlab("Age") + ylab("Mean GM CBF")+ geom_boxplot(width=0.5) + theme(text = element_text(size = 20))